...
class Cell:
    ...
    def create_btn_object(self, location):
        ...
        # Button-3 is for a mouse with wheel, otherwise Button-2 or 4 etc.
    ...
    def show_cell(self):
        if not self.is_opened:
            Cell.cell_count -= 1
            # Logic to show cell if not a mine
            self.cell_btn_object.configure(text = self.surrounded_cells_mines_length)
            # Replace text of cell count label with the updated count
            if Cell.cell_count_label_object:
                Cell.cell_count_label_object.configure(
                    text=f"Cells Left: {Cell.cell_count}"
                )
            # If this is a mine candidate, then for safety, we should
    ...
    def randomize_mines():
        picked_cells = random.sample(
            Cell.all, minesweeper_settings.MINES_COUNT
            )
